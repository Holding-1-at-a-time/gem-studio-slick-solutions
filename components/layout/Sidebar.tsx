...
const NavLink: React.FC<{ href: string; icon: React.ReactNode; children: React.ReactNode }> = ({ href, icon, children }) => (
    <a href={href} className="flex items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary hover:bg-secondary">
        {icon}
        {children}
    </a>
);
...
const Sidebar: React.FC = () => {
    ...
    const role = user?.publicMetadata?.role === 'admin' ? 'admin' : membership?.role;
    ...
    let navLinks: React.ReactNode[] = [];

    if (role === 'admin') {
        navLinks = [
            <NavLink key="dash" href="/dashboard" icon={ICONS.dashboard}>Dashboard</NavLink>,
            <NavLink key="tenants" href="/dashboard" icon={ICONS.tenants}>Tenants</NavLink>,
        ];
    } else if (role === 'org:detailer') {
        navLinks = [
            <NavLink key="dash" href="/dashboard" icon={ICONS.dashboard}>Dashboard</NavLink>,
            <NavLink key="kb" href="/kb" icon={ICONS.kb}>Knowledge Base</NavLink>,
        ];
    } else if (role === 'org:client') {
        navLinks = [
             <NavLink key="dash" href="/dashboard" icon={ICONS.dashboard}>My Appointments</NavLink>,
             <NavLink key="kb" href="/kb" icon={ICONS.kb}>Knowledge Base</NavLink>,
        ];
    }
